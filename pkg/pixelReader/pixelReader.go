...
func parseAndCompareMatrixes(startingPoint int, filePiece []byte, ch chan int) {
	...
	for i := 0; i < len(filePiece); i += PixelSize {
		pixel := Pixel{RGB: [PixelSize]int{int(filePiece[i]), int(filePiece[i+1]), int(filePiece[i+2])}}
...
func GetTopThreeString() string {
	return fmt.Sprintf("BEST: %v\nSECOND: %v\nTHIRD: %v\n", Best, Second, Third)
}
